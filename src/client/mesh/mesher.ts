  ...
  for (let ly = 0; ly < S; ly++) {
    for (let lz = 0; lz < S; lz++) {
      for (let lx = 0; lx < S; lx++) {
        ...
        // CUBE（FLUID / CUSTOM 在 M7 补，先按立方体渲染以免整块消失）
        for (let f = 0; f < 6; f++) {
          const face = FACES[f]!;
          const nx = px + face.nx;
          const ny = py + face.ny;
          const nz = pz + face.nz;
          const neighborState = blocks[paddedIndex(nx, ny, nz)]!;
          if (!faceVisible(t, id, neighborState)) continue;

          buf.ensure(1);
          const layerTex = t.faceLayer[id * 6 + face.face]!;
          // 按面决定要不要染色：草方块侧面的泥土部分不能被染绿
          const faceTint = ((t.tintFaces[id]! >> face.face) & 1) === 1 ? tint : 0;
          const base = buf.quads * 4;

          // 面所在平面的两条切向就是 face.u / face.v
          const ao = [0, 0, 0, 0];
          const lightVals = [0, 0, 0, 0];
          ...
          for (let c = 0; c < 4; c++) {
            ...
            // 平滑光照：对四个非实心采样点取平均。全实心时退回面法线方向的值。
            let skySum = 0;
            let blockSum = 0;
            let n = 0;
            ...
            if (n === 0) {
              const lv = light[paddedIndex(nx, ny, nz)]!;
              lightVals[c] = lv;
            } else {
              lightVals[c] = ((Math.round(skySum / n) & 0xf) << 4) | (Math.round(blockSum / n) & 0xf);
            }
          }

          // 四边形翻转：AO 对角和不等时改变三角剖分方向，
          // 否则会在有遮蔽的角上看到明显的对角线接缝
          const flip = ao[0]! + ao[2]! > ao[1]! + ao[3]!;

          for (let c = 0; c < 4; c++) {
            const addU = c === 1 || c === 2 ? 1 : 0;
            const addV = c === 2 || c === 3 ? 1 : 0;
            const vx = lx + face.ox + face.ux * addU + face.vx * addV;
            const vy = ly + face.oy + face.uy * addU + face.vy * addV;
            const vz = lz + face.oz + face.uz * addU + face.vz * addV;
            const uv = FACE_UV[c]!;
